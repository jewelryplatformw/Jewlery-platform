...
export default function MetalAnalytics({ metalStock }: Props) {
  ...
  const gold24k = typeof gold?.spot_price === 'number' && gold.spot_price > 0 ? gold.spot_price : 412.00;
  const silver24k = typeof silver?.spot_price === 'number' && silver.spot_price > 0 ? silver.spot_price : 4.50;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div className="p-4 rounded-xl border border-amber-500/20 bg-amber-500/5">
          <div className="text-amber-400 text-sm font-medium">{t.gold24kLabel}</div>
          <div className="text-2xl font-bold text-white mt-1">{gold24k.toFixed(2)} {t.tnd}</div>
        </div>
        <div className="p-4 rounded-xl border border-amber-500/20 bg-amber-500/5">
          <div className="text-amber-300 text-sm font-medium">{t.gold18kLabel}</div>
          <div className="text-2xl font-bold text-white mt-1">{gold18k.toFixed(2)} {t.tnd}</div>
        </div>
        <div className="p-4 rounded-xl border border-slate-400/20 bg-slate-400/5">
          <div className="text-slate-300 text-sm font-medium">{t.pureSilverLabel}</div>
          <div className="text-2xl font-bold text-white mt-1">{silver24k.toFixed(2)} {t.tnd}</div>
        </div>
      </div>
